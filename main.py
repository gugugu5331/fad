...
def main(args: argparse.Namespace):
    ...
    if feature_device_cuda:
        if args.cache_features:
            if train_workers:
                print(f"[info] caching features on CUDA; forcing num_workers from {train_workers} to 0 to keep cached GPU tensors in the main process.")
                train_workers = 0
        elif train_workers:
