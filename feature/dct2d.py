...
def compute_feature(raw_data, sr):
    X = pad(raw_data)
    X = mel_2D(X, fs=sr, nfilts=60, nfft=1024, win_len=0.030, win_hop=0.010)
    return X
...
def mel_2D(sig,
    ...
    #  -> x linear-fbanks
    mel_fbanks_mat, _ = mel_filter_banks(nfilts=nfilts,
                                            nfft=nfft,
                                            fs=fs,
                                            low_freq=low_freq,
                                            high_freq=high_freq,
    ...
    log_features = np.log10(features + 2.2204e-16)  # (len, nfilts=128)

    #  -> 2D-DCT(.)
    #lfccs = dct(log_features, type=dct_type, norm='ortho', axis=1)[:, :num_ceps]
    #print(log_features.shape)
    mel2D = dctn(log_features, type=dct_type)[:, :40]  # (len, nfilts=128)
