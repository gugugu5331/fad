...
def ce_loss(p, alpha, c, global_step, annealing_step):
    S = torch.sum(alpha, dim=1, keepdim=True)#alpha 32 1 2  y 32
    E = alpha - 1
    label = F.one_hot(p, num_classes=c) #32 2
    A = torch.sum(label * (torch.digamma(S) - torch.digamma(alpha)), dim=1, keepdim=True)

    annealing_coef = min(1, global_step / annealing_step)

    alp = E * (1 - label) + 1# 32 32 2  E 32 1 2   label 32 2
...
class fushion_decision(nn.Module):
    ...
    def forward(self, X, y, global_step):
        ...
        loss = 0
        alpha = dict()
        for v_num in range(len(X)):  # y 32 a
