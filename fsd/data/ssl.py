...
class ASVspoof19TrainDataset(Dataset):
    ...
    def _load_cached_item(self, utt: str) -> Optional[Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]]:
        if self.disk_cache_dir is None:
            return None
        cache_file = self.disk_cache_dir / f"{utt}.pt"
        if not cache_file.exists():
            return None
        data = torch.load(cache_file, map_location="cpu", weights_only=True)
        label_val = data.get("label")
        if label_val is None:
            raise KeyError(f"Cached feature for {utt} is missing label")
        return (
            data["wave"],
            data["lfcc"],
            data["dct2d"],
            torch.tensor(int(label_val), dtype=torch.long),
        )
    ...
    def cache_all_features(
        ...
        *,
        ...
        cache_workers: int | None = 0,
    ):
        """Load/compute all features once and keep them in memory.

        Notes:
          - This is meant to reduce disk I/O / feature recomputation during training.
          - `cache_device="cpu"` avoids GPU OOM for large datasets.
        """
        if self.cache_data is not None:
            return

        if cache_workers is None:
            worker_count = getattr(self.args, "num_workers", 0) if self.args else 0
        else:
            worker_count = int(cache_workers)
        print(f"[info] Caching features with {worker_count} workers...")

        # 1. 保存原始设备配置，并临时强制切到 CPU
        #    这样 Worker 进程就不会尝试访问 CUDA，避免 fork 错误
        original_device = self.feature_device
        self.feature_device = "cpu"
        if hasattr(self, 'lfcc_tf'):
            self.lfcc_tf = self.lfcc_tf.cpu()

        # 2. 创建 DataLoader (此时 self 里的配置都是 CPU)
        #    Worker 会在 CPU 上完成 _compute_item 的所有计算
        ...
        # 3. 主进程循环获取数据
        try:
            for wave, lfcc, dct2d, label in tqdm(temp_loader, desc="Caching features", total=len(self)):
                cached.append(
                    (
                        wave.to(target_device),
                        lfcc.to(target_device),
                        dct2d.to(target_device),
                        label.to(target_device),
                    )
                )
        finally:
            # 4. 无论是否成功，最后都要恢复原始配置
            self.feature_device = original_device
            if hasattr(self, 'lfcc_tf') and str(original_device).startswith("cuda"):
                self.lfcc_tf = self.lfcc_tf.to(original_device)
    ...
    def __getitem__(self, idx: int):
        utt, _ = self.entries[idx]
        if self.cache_data is not None:
            sample = self.cache_data[idx]
        else:
            cached = self._load_cached_item(utt)
            sample = cached if cached is not None else self._compute_item(idx)
...
class ASVspoof21EvalDataset(Dataset):
    ...
    def _load_cached_item(self, utt: str):
        if self.disk_cache_dir is None:
            return None
        cache_file = self.disk_cache_dir / f"{utt}.pt"
        if not cache_file.exists():
